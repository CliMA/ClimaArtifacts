...
Given a text file with a URL to download a file on each line, download each file
serially from each URL in `filepath_to_txt_file` and save them to `output_dir`.
"""
...
# Generate the datasets for the artifact
